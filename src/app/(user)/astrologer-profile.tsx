...
import { MOCK_ASTROLOGERS, MOCK_SERVICES } from '@/src/mock/data';
...
export default function AstrologerProfileScreen() {
  ...
  const astrologer = MOCK_ASTROLOGERS.find(a => a.id === id) || MOCK_ASTROLOGERS[0];
  const services = MOCK_SERVICES.filter(s => s.astrologerId === astrologer.id);
    ...
    <View style={styles.root}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.tabs}>
          ...
          >
            <Text style={[styles.tabText, activeTab === 'services' && styles.tabTextActive]}>Services ({services.length})</Text>
          </TouchableOpacity>
        </View>
              ...
              {services.map((service) => (
                <View key={service.id} style={styles.serviceCard}>
                  ...
                  <View style={styles.serviceChips}>
                    ...
                    <View style={styles.chip}><Text style={styles.chipText}>{service.callType === 'VIDEO' ? '📹 Video' : '📞 Voice'}</Text></View>
                  </View>
                  <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  avatarCircle: { width: 90, height: 90, borderRadius: 45, backgroundColor: 'rgba(255,255,255,0.2)', alignItems: 'center', justifyContent: 'center' },
  ...
  content: { padding: 16 },
  aboutSection: { gap: 12 },
  sectionTitle: { fontSize: 15, fontWeight: '800', color: '#1A1A2E', marginTop: 8 },
  bio: { fontSize: 14, color: '#555', lineHeight: 22 },
  tags: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  tag: { backgroundColor: '#F3E8FF', borderRadius: 20, paddingHorizontal: 12, paddingVertical: 6, borderWidth: 1, borderColor: '#9d0399' },
  tagText: { color: '#9d0399', fontSize: 12, fontWeight: '600' },
  servicesSection: { gap: 12 },
  serviceCard: { backgroundColor: '#FFF', borderRadius: 14, padding: 16, borderWidth: 1, borderColor: '#EDE9FF', gap: 10, elevation: 1 },
  serviceTop: { flexDirection: 'row', gap: 12, alignItems: 'flex-start' },
  serviceName: { fontSize: 15, fontWeight: '800', color: '#1A1A2E', marginBottom: 4 },
  ...
  bookBtn: { backgroundColor: '#9d0399', borderRadius: 12, paddingVertical: 12, alignItems: 'center' },

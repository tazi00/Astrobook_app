...
import { MOCK_ASTROLOGERS, MOCK_SERVICES, MOCK_SLOTS } from '@/src/mock/data';
...
export default function BookSlotScreen() {
  ...
  const { astroId, serviceId } = useLocalSearchParams<{ astroId: string; serviceId: string }>();

  const astrologer = MOCK_ASTROLOGERS.find(a => a.id === astroId) || MOCK_ASTROLOGERS[0];
    ...
    <View style={styles.root}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.serviceCard}>
          <View style={[styles.serviceAvatar, { backgroundColor: astrologer.color }]}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <View style={styles.serviceChips}>
              <View style={styles.chip}><Text style={styles.chipText}>⏱ {service.durationMins} min</Text></View>
              <View style={styles.chip}><Text style={styles.chipText}>{service.callType === 'VIDEO' ? '📹' : '📞'} {service.callType}</Text></View>
              <View style={[styles.chip, styles.priceChip]}><Text style={styles.priceText}>₹{service.price}</Text></View>
            </View>
          </View>
        </View>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.dateScroll}>
            {dates.map((date, index) => (
              ...
              >
                ...
                <Text style={[styles.dateMon, selectedDate === index && styles.dateTextActive]}>
...
const styles = StyleSheet.create({
  ...
  serviceCard: { flexDirection: 'row', backgroundColor: '#FFF', padding: 16, gap: 14, borderBottomWidth: 1, borderBottomColor: '#EDE9FF' },
  serviceAvatar: { width: 64, height: 64, borderRadius: 32, alignItems: 'center', justifyContent: 'center' },
  serviceName: { fontSize: 15, fontWeight: '800', color: '#1A1A2E', marginBottom: 2 },
  astroName: { fontSize: 12, color: '#9d0399', fontWeight: '600', marginBottom: 6 },
  serviceChips: { flexDirection: 'row', gap: 6, flexWrap: 'wrap' },
  chip: { backgroundColor: '#F5F0FF', borderRadius: 8, paddingHorizontal: 8, paddingVertical: 3 },
  chipText: { fontSize: 10, color: '#6B21A8', fontWeight: '600' },
  priceChip: { backgroundColor: '#9d039918', borderWidth: 1, borderColor: '#9d0399' },
  priceText: { fontSize: 12, fontWeight: '800', color: '#9d0399' },
  content: { padding: 16, gap: 12 },
  sectionTitle: { fontSize: 15, fontWeight: '800', color: '#1A1A2E', marginTop: 4 },
  dateScroll: { marginHorizontal: -4 },
  dateCard: { alignItems: 'center', padding: 12, marginHorizontal: 4, borderRadius: 12, backgroundColor: '#FFF', borderWidth: 1.5, borderColor: '#EDE9FF', minWidth: 60 },
  ...
  slotBtn: { paddingHorizontal: 16, paddingVertical: 10, borderRadius: 10, backgroundColor: '#FFF', borderWidth: 1.5, borderColor: '#EDE9FF' },
  ...
  bottomBar: {
    ...
    borderTopWidth: 1, borderTopColor: '#EDE9FF', elevation: 10,
  },
  totalLabel: { fontSize: 12, color: '#AAA' },
  totalPrice: { fontSize: 22, fontWeight: '800', color: '#1A1A2E' },
  bookBtn: { backgroundColor: '#9d0399', borderRadius: 14, paddingHorizontal: 28, paddingVertical: 14 },
